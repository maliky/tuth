...
* [2025-11-19 mer. 09:56] Preparing import data
: Context :
Scan attentively the models of academics finance people registry shared spaces and timetable.
Look at app/shared/management/commands create_states, create_test_users, load_roles,  run them
Look at app/shared/management/commands/ import_resources . See how I use the import-export module to populate the db.

: Goal :
The goal of the task is to identify one or several (not more than 10) effective(s) csv or tsv structures to populate the database given sources in mixed format.  The later will discribed later.
I will then do some data wrangling to massage the sources files into the identified structures that can be imported using existing or enhanced command line management scripts

** Import-Friendly Schemas

Below are nine CSV/TSV layouts (≤10 as requested) that align with the existing django-import-export resources and cover academics, people, spaces, timetable, registry, and finance. Each file can be produced from mixed upstream sources, then loaded with the current management commands (create_states, load_roles, create_test_users, import_resources / import_resources_individualy) or thin wrappers around those resources.

*** academics_colleges_departments.csv
- Targets :: CollegeResource, DepartmentResource (app/academics/admin/resources.py:86).
- Columns :: college_code, college_long_name, course_dept, department_title.
- Load order :: import colleges first (dedupe on college_code), then departments referencing the same rows (department → college). Works well when extracted 
from a campus ERP or accreditation catalog.

*** academics_courses.csv
- Targets :: CourseResource (app/academics/admin/resources.py:119).
- Columns :: course_dept, course_no, course_title, prerequisites (comma-separated course codes).

Ensures every course exists before linking curricula/sections.

*** academics_curricula.csv
- Targets :: CurriculumResource (app/academics/admin/resources.py:19).
- Columns :: curriculum (short name), title, college_code, list_courses (semicolon-separated course codes).

Supports merge/replace semantics already built into the resource for easy revisions

*** academics_curriculum_courses.csv
- Targets :: CurriculumCourseResource (app/academics/admin/resources.py:167).
- Columns :: curriculum, course_dept, course_no, credit_hours.

Optional but handy when credit hours differ from the catalog default; use
after importing base curricula.

*** people_directory_faculty.csv

- Targets :: DirectoryContactResource + FacultyResource (app/people/admin/resources.py:14).
- Columns :: faculty (full name), username, email, college_code, title.

Produces Staff + Faculty records and auto-assigns the faculty group/role via UserRole logic.

*** people_students.csv
- Targets :: StudentResource (app/people/admin/resources.py:64).
- Columns :: student_id, student_name (or split name fields), major (curriculum code), entry_semester, current_enrolled_sem, birth_date (YYYY-MM-DD HH:MM:SS), nationality, marital_status, gender.

Works in tandem with SemesterResource so the semester widgets resolve; include student_email if you’d like to extend UserStudentWidget.

*** spaces_rooms.csv

- Targets :: RoomResource (app/spaces/admin/resources.py:7).
- Columns :: space (building code), room, standard_capacity, exam_capacity.

Use this once per physical inventory extract; spaces auto-created by the widget.

*** timetable_semesters_sections.csv

- Targets :: SemesterResource (app/timetable/admin/resources/core.py:9) followed by SectionResource (app/timetable/admin/resources/section.py:8).
- Semesters columns :: academic_year (e.g., 25-26), semester_no, start_date, end_date.
- Sections columns :: academic_year, semester_no, curriculum, course_dept, course_no, section_no, faculty.

Keep in one sheet with two tabs or two CSVs; import semesters first so the SemesterWidget resolves.

*** timetable_sessions.csv
- Targets :: ScheduleResource + SecSessionResource (app/timetable/admin/resources/session.py:8).
- Columns :: section_no, weekday, start_time (HH:MM), end_time, room.

Optionally add schedule_code if you want to re-use identical meetings for multiple sections.

*** registry_finance_registrations.tsv
/TSV recommended because it often mixes  numeric IDs with text containing commas/

- Targets :: RegistrationResource (app/people/admin/resources.py:106) and finance models (app/finance/models/payment.py:1).
- Columns :: record_type (registration or payment), student_id, section_no, status (for registrations), or invoice_number, amount_paid, payment_method, clearance_status, recorded_by_username (for payments).

A simple loader can split the TSV into two tablib datasets: registrations feed import_resources, payments call a small helper that creates/updates Invoice/Payment rows after create_states populates FeeType, PaymentMethod, etc.

** Execution Flow

1. Run python manage.py create_states, load_roles, and (optionally)
 create_test_users so enumerations, Guardian roles, and sample accounts exist (app/shared/management/commands/*.py).
2. Import foundation tables in dependency order: colleges/departments → courses → curricula → curriculum_courses.
3. Load people (people_directory_faculty.csv, people_students.csv) so references in sections and payments resolve.
4. Seed spaces, semesters, sections, sessions.
5. Finish with transactional data (registry_finance_registrations.tsv), leveragin import_resources for the tablib datasets (the command already loops over  Student, Faculty, Room, Schedule, Course, Semester, CurriculumCourse, Section,  SecSession per app/shared/management/commands/import_resources.py:32).

This set keeps the number of files small, mirrors the existing admin resources, and lets you massage any upstream spreadsheets into predictable shapes for automated imports.
...
* <2025-11-19 mer. 10:36> updating readme
  

* [2025-11-19 mer. 09:44] fixed basic deployment
• Preprod Fixes

  - Added the missing website/portal_login.html view template so /auth/login/ no
    longer raises TemplateDoesNotExist; the page now renders the login form with the    expected copy and inputs (app/website/templates/website/portal_login.html:1).
  - Allowed local health probes by expanding the host list in .env-preprod to
    tusis.koba.sarl,127.0.0.1,localhost, preventing the earlier DisallowedHost 400
    when curling the container directly (.env-preprod:1).
  - Rebuilt the stack (docker-compose -f docker-compose-preprod.yml up -d --build)
    so the new template and env tweaks are baked into the running web container;
    confirmed /auth/login/ now returns HTTP 200 via curl http://127.0.0.1:18000/
    auth/login/.

  Tests

  - DJANGO_SETTINGS_MODULE=app.settings pytest -m selenium tests/selenium/
    test_landing_page.py – passed (2 tests, 2.55s).
  - DJANGO_SETTINGS_MODULE=app.settings pytest -m selenium tests/selenium/
    test_portal_roles.py – passed (13 tests, 33.63s).
    These suites spin up Django’s live server locally; they exercise the same code
    paths as preprod and verified the login link and role dashboards end-to-end.

  Next steps

  1. Hit https://tusis.koba.sarl/auth/login/ (after HTTP Basic auth) to ensure the
     rendered form matches expectations.
  2. If you need Selenium to drive the actual preprod URL instead of the built-in
     live server, we can introduce a configurable base URL in the tests; let me know     and I can adjust the fixtures accordingly.



* <2025-11-18 mar. 21:46> deployment
codex resume 019a7a7e-5d92-7453-98a8-28c20b165b2d 
• Deployment State

  - Updated docker-compose-preprod.yml:7 to only run Postgres + the Django/Gunicorn
    app, mount host static/ & media/, and expose Gunicorn on 127.0.0.1:18000 for
    your native Nginx. Removed the old containerized Nginx service.
  - Added django-extensions==3.2.3 to the production dependencies
    (requirements.txt:1) so the app boots cleanly with INSTALLED_APPS.
  - Brought the stack up via newgrp docker docker-compose -f docker-compose-
    preprod.yml up -d --build --remove-orphans; DB and web containers are now
    running (see docker-compose … ps above).
  - Entry-point logs show migrations ran through the auth/admin/guardian apps and
    collectstatic synced static/ (docker-compose … logs web).

  Next steps

  1. Since newgrp docker was needed to use the daemon, log out/in (or reboot) so the     docker group sticks, otherwise every Docker command must be wrapped in newgrp
     docker <<'EOF' … EOF.
  2. Hit the site through native Nginx to confirm proxying + HTTP basic auth: curl
     -u <user>:<pass> https://tusis.koba.sarl/healthz.
  3. Tail the web logs when testing (docker-compose -f docker-compose-preprod.yml
     logs -f web) and use docker-compose … exec web python manage.py createsuperuser     etc. for further setup.
  4. Keep .env-preprod updated with real secrets/allowed hosts before exposing the
     site more broadly.



* <2025-11-18 mar. 19:19> native nginx, dockerized django and  docker
What you did is very good, but I only want the posgresql DB and the django engine to be dockerized. I have already a native nginx that I want to use.  So review your proposition. I have have created already the htpasswd in /etc/nginx I only need a clean tusis.koba.sarl.conf file for nginx beside teh deployment file for docker compose

** • Changes

  - Reworked the pre-prod compose stack so it now runs only PostgreSQL and the
    Django/Gunicorn app, mounts the repo’s static/ + media/ directories, and
    publishes the backend as 127.0.0.1:18000 for native Nginx (docker-compose-
    preprod.yml:7).
  - Updated the entrypoint to wait for Postgres, collect static assets, and
    bind Gunicorn to 0.0.0.0:${GUNICORN_PORT:-8000} instead of a Unix socket,
    plus added the new port option in .env-preprod.example (deploy/preprod/
    entrypoint.sh:8, .env-preprod.example:1).
  - Added a drop-in site file for the host’s Nginx that enforces Basic Auth, serves
    static/media from /home/jil/Tusis/Tusis_app, and proxies to the containerised
    backend (infra/nginx/tusis.koba.sarl.conf:1) while removing the old dockerised
    Nginx assets (infra/preprod/nginx/*).
  - Rewrote the README pre-prod section with step-by-step guidance for this split
    setup (Dockerised DB/app + native Nginx) and the new health-check/log/backup
    commands (README.org:169).
  ...
  1. cp .env-preprod.example .env-preprod, fill real secrets, and run docker-compose     -f docker-compose-preprod.yml up -d --build.
  2. Copy infra/nginx/tusis.koba.sarl.conf into /etc/nginx/sites-available/, adjust
     certificate paths if needed, enable it, and reload Nginx so it proxies to
     127.0.0.1:18000.
