...
* <2025-11-12 mer. 14:23>
  › expand the view routing map2 in journal with explicitly showing user with the exact
  same role_name as in the perms.py file.  if possible find a way to show the
  hiaerachy of some roles among the users.  I understand that all the staf roles have
  now acces to admin


 should Dean have access to chair dashboard a list of their chairs ?

 In the error page remove the link back to login. replace it with landing

 I do not see the difference between staff/dashboard and staff/<role> ?

 The enrollement officer can see the enrollment dashboard maybe the choice of other dashboar should be what is shown in staff/dashboardS and the user own live under staff/<role>. there is redundency to remove here.
*** Enrollement
    Pick an ID and Tusis will send you straight to the admin edit screen.
- The form should show directly in the little card, do use an intermediary screen.  The form should be a search field filtering down id based on characters types,  the search field should also be id and full name of the students.
