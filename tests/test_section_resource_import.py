...
def make_dataset(course, semester, headers=None):
    if headers is None:
        headers = [
            "academic_year",
            "course_name",
...
def test_missing_required_header_triggers_error(missing, course, semester):
    ...
    must report errors.
    """
    headers = make_dataset(course, semester).headers
    headers.remove(missing)
    ds = make_dataset(course, semester, headers)

    # strip blank headers if any
    SectionResource().before_import(ds, using_transactions=False, dry_run=True)
    result = SectionResource().import_data(ds, dry_run=True)
...
def test_successful_import_creates_section(course, semester):
    ...
    """
    ds = make_dataset(course, semester)

    # dry_run should pass
    dry = SectionResource().import_data(ds, dry_run=True)

    assert not dry.has_errors()

    # real import
    imp = SectionResource().import_data(ds, dry_run=False)
    assert isinstance(imp, results.Result)
