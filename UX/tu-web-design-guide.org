...
* Shared Portal UI Inheritance Model
The portal uses one visual language. The student UI is the reference implementation. Staff, dean, finance, registrar, faculty, chair, and other internal roles inherit the same base shell and component language rather than switching to a distinct admin product.

** Inheritance Rules
- =portal-base=: abstract shell shared by every authenticated user. It owns the left rail, sidebar header, content header, feedback region, content spacing, and base component slots.
- =student-portal=: first concrete implementation and visual reference. It defines the tone, card rhythm, spacing scale, badge treatment, table behavior, and primary call-to-action hierarchy.
- =staff-portal=: extends =portal-base= and reuses student visual tokens. It may increase information density and expose role-specific tools, but it must not change the overall mood, palette, or interaction model.
- Role workspaces such as =finance=, =dean=, =registrar=, and =faculty= extend =staff-portal= and contribute task modules only. They should not introduce their own shells.
- When documentation conflicts on visual behavior, this guide defines the intended UI.

** MWE Implementation Direction
- The public Tusis landing page is an official portal gateway, not a broad marketing website. It should reassure users that they are entering a Tubman University service and route them quickly to login, role selection, and support.
- Use the live university website for public terminology, contact details, and institutional cues. Do not copy external photos or social-media assets into Tusis unless usage rights are explicit.
- Until approved university photos are available, use local static assets and brand-led composition rather than placeholder social images.
- Student, staff, dean, registrar, and finance screens should now inherit the shared =portal-base= shell. Any new backstage workflow should start from =staff-portal= and add task modules only.
- Django admin remains available for privileged users, but it is an advanced path. It should not be presented as the main user journey on public or staff landing surfaces.

* Staff Workspace Variant
Staff workspaces remain desktop-capable, but they should still feel like the student portal. The difference is density and task scope, not theme identity.

** Palette & Tone
- Reuse the student portal palette: light content canvas, dark sidebar, green header band, white cards, and gold accents.
- Staff may add semantic emphasis for operational states, but they should not switch to a separate dark admin theme.
- Preserve the same card radius, shadow depth, badge language, and spacing scale used in the student portal.

** Grid & Navigation
- Keep the same left-rail shell as student. The sidebar remains the primary navigation anchor.
- The sidebar header contains the workspace identity and the role switcher.
- Sidebar links are task-oriented. Prefer labels such as =Payments=, =Approvals=, =Records=, and =My Sections= over generic =Dashboard= or =Roles= entries.
- Breadcrumbs are optional secondary context, not a replacement for the sidebar.
- Use the same page rhythm as the student portal: prominent page header, KPI row when useful, then stacked white content sections.

** Components
- KPI cards, section cards, filters, badges, tables, empty states, and action buttons inherit the student portal component language.
- Forms use the same input borders, labels, help text, validation tone, and submit hierarchy as student pages.
- Dense operational screens may add bulk actions, inline filters, and collapsible tables, but they stay inside the same card and table system.
- Finance screens must inherit staff templates and skin any admin-origin widgets so they match portal forms.
- Notifications use the same toast and inline feedback language across student and staff surfaces.

** Responsiveness
- Staff views can use wider data layouts, but they still collapse through the shared responsive rules from =portal-base=.
- On mobile and tablet, preserve the same sidebar and header behavior as the student portal. Do not introduce a separate top-nav-only admin pattern.
- Critical actions must remain touch-accessible and keyboard reachable.

** Operational Notes
- Formalize shared CSS custom properties and shared template blocks at the =portal-base= layer before expanding staff screens.
- Extract reusable student components first, then migrate staff roles to consume them.
- Finance, dean, registrar, faculty, chair, and other staff roles should vary by content modules and permissions, not by visual skin.
- Keep charts and heavy widgets lightweight and aligned with the portal rhythm.
