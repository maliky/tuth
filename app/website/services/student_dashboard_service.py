...
from typing import DefaultDict, Iterable, Optional, TypedDict
...
from app.academics.models.requirement_group import ReqKind
...
class AjaxMessageT(TypedDict):
    """Message payload returned by AJAX registration mutations."""

    level: str
    text: str


class PrereqChipT(TypedDict):
    """Template payload for a prerequisite status chip."""

    label: str
    met: bool


class SectionOptionT(TypedDict):
    """Template payload for a selectable course section."""
...
class CourseCardT(TypedDict):
    ...
    prerequisites: list[PrereqChipT]
    sections: list[SectionOptionT]
    level_hint: str
    status_label: str
    status_class: str
    locked_until: str
...
def student_dashboard_response(request: HttpRequest) -> HttpResponse:  # noqa: C901
    ...
    ajax_messages: list[AjaxMessageT] = []
    ...
    dashboard_sections_qs: QuerySet[Section]
    if curriculum_course_ids:
        dashboard_sections_qs = Section.objects.filter(
        ...
        dashboard_sections_qs = dashboard_sections_qs.prefetch_related(
            "sessions__schedule"
        )
        if semester:
            dashboard_sections_qs = dashboard_sections_qs.filter(semester=semester)
    else:
        dashboard_sections_qs = Section.objects.none()

    sections_by_course: dict[int, list[Section]] = defaultdict(list)
    for section in dashboard_sections_qs:
    ...
    prereq_map: dict[int, list[PrereqChipT]] = defaultdict(list)

    def _append_prereq_chip(course_id: int, label: str, met: bool) -> None:
        """Append one prerequisite chip without duplicating its label."""
        chips = prereq_map[course_id]
        if any(chip["label"] == label for chip in chips):
            return
        chips.append({"label": label, "met": met})
    ...
    for prereq in prereqs:
        ...
        _append_prereq_chip(
            course_id,
            prereq_course.short_code or prereq_course.code,
            prereq_course.id in passed_course_ids,
        )
    for curriculum_course in curriculum_courses:
        for req_group in curriculum_course.requirement_groups.all():
            if req_group.kind not in {ReqKind.PREREQ_ALL, ReqKind.PREREQ_ANY}:
                continue
            for member in req_group.members.all():
                required_course = member.required_course
                _append_prereq_chip(
                    curriculum_course.course_id,
                    required_course.short_code or required_course.code,
                    required_course.id in passed_course_ids,
                )

    available_courses: list[CourseCardT] = []
    registered_courses: list[CourseCardT] = []
    locked_courses: list[CourseCardT] = []
    # Keep a dedicated list for canceled courses to drive the gray area UI.
    canceled_courses: list[CourseCardT] = []
    for cc in curriculum_courses:
        ...
        serialized_sections: list[SectionOptionT] = [
            {
                "id": section.id,
    ...
    for grade in grades:
        ...
        grade_group = semester_grade_lookup.get(semester_id)
        if grade_group is None:
            ...
            semester_grade_lookup[semester_id] = grade_group
            semester_grade_groups.append(grade_group)
        ...
        grade_group["courses"].append(row)
    ...
    for grade_group in semester_grade_groups:
        ...
        if credit_total:
            ...
            grade_group["gpa"] = f"{gpa_value:.2f}"
        grade_group["credits_total"] = semester_credit_totals.get(semester_id, 0)

    semester_grade_groups.sort(
        key=lambda grade_group: semester_sort_info.get(
            grade_group["semester_id"], (date.min, 0)
        ),
        reverse=True,
    )
    for grade_group in semester_grade_groups:
        if grade_group["semester_id"] in cleared_invoice_semester_ids:
            grade_group["receipt_url"] = reverse(
                "std_payment_receipt",
                args=[grade_group["semester_id"]],
            )
...
__all__ = ["student_dashboard_response"]
