...
class SectionWidget(widgets.ForeignKeyWidget):
    """Parse the necessary CSV columns to get or create a Section."""
...
class SectionCodeWidget(widgets.Widget):
    ...
    def clean(
        ...
        **kwargs,
        ...
        faculty_value = row.get("faculty", "").strip()
        faculty = self.fac_w.clean(value=faculty_value, row=row)
